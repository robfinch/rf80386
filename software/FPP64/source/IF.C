...
void difskip(int s)
{
   ...
   while(!feof(fin))
   {
      ...
      */
     if (syntax ? NextNonSpace(0) != '.' : NextNonSpace(0) != '#') {
       ScanPastEOL();
       continue;
     }
...
void repskip(int s)
{
  ...
  while (!feof(fin))
  {
    ...
    // Break when an endr is encountered.
    if (!strncmp(inptr, "endr", 4))
      return;
  }
  err(12);
}
...
void* dif(int opt)
{
  ...
  inptr = SearchAndSub(inbuf, inptr);      // perform any macro substitutions
  switch (opt) {
