...
/* -----------------------------------------------------------------------------
  ...
  Returns:
    The length of the expanded macro.
----------------------------------------------------------------------------- */
...
int SubParmMacro(SDef *p, int opt)
{
   ...
   if (opt == 0) {
     ...
     c = NextNonSpace(opt ? 0 : 1);                       // Skip past closing ')'
     if (c != ')' && syntax == 0) {
       unNextCh();
       err(3);     // missing ')'
     }
     if (ArgCount != nArgs && nArgs > 0)                    // Check that argument count matches
       err(14, nArgs);
   }
   else {
     if (p->nArgs < 0)
       ...
       ArgCount = p->nArgs;
     for (xx = 0; xx < ArgCount; xx++)
       Args[xx] = *p->parms[xx];
   }

   // Substitute arguments into macro body
   // Need to allocate more to allow arguments to be substituted.
   ...
   {
      tp = SubMacroArg(bdy, xx+1, Args[xx].def);        // Substitute argument into body
      q = tp;
      /*
      do {
        q = strstr(q, ".endr[.minst]");
        if (q) {
          char buf[20];
          c = q[12];
          sprintf_s(buf, sizeof(buf), ".endr[%.5d ]", minst);
          memcpy_s(q, sizeof(buf), buf, strlen(buf));
          q += strlen(buf);
        }
      } while (q);
      */
...
/* -----------------------------------------------------------------------------
  ...
  Returns:
    A new value for the input pointer. SearchAndSub may alter the input
  buffer location.
----------------------------------------------------------------------------- */

char* SearchAndSub(buf_t *buf, char* ip)
{
